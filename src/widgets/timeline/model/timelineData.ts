...
export const TIMELINE_DATA: ITimelineItem[] = [
  ...
  },
  {
    ...
    description: [
      "Завершив задачи на армейском «фронтенде», после возвращения я всерьёз взялся за прокачку себя как разработчика. Мотивация была предельно ясной - найти первую работу в IT.",
  ...
  },
  {
    ...
    description: [
      ...
      "Начал верстать страничку... Но главной целью было понять экосистему React и её возможности на базовом уровне",
    ],
    imageUrls: [
  ...
  },
  {
    ...
    description: [
      "Потрогав React. Я, решительно настроился себя на Fullstack проект.",
      "Это MPA. Здесь есть база данных и авторизация (правда, 1979 года). Проект был нацелен на тренировку REST API + CRUD.",
      "Угадаете, что это? Данное словосочетание ассоциируется у новичков только с одним - TODO-LIST!",
  ...
  },
  {
    ...
    description: [
      ...
      "Где-то в конце мая ко мне пришла идея собрать команду разработчиков и дизайнера для его разработки. Всё это было нацелено на симуляцию командного опыта, как того требует IT-среда.",
      "Потратив огромное количество времени на тот момент, не очень хорошую документацию проекта и разного рода ссылки для приглашения, я стал рассылать всё по всем популярным форумам, YouTube-роликам и телеграм-чатам.",
      ...
      "На протяжении разработки к нам приходили разработчики разных уровней — от балансировщиков баз данных до Tech Lead'ов. Значительная часть времени была потрачена на нормализацию процессов, поэтому в разработке мы не преуспели, а проект пришлось заморозить в связи с необходимостью найти работу.",
    ],
    stack: [
      "React",
      "Next.js",
  ...
  },
  {
    ...
    description: [
      "На мою удачу, буквально сразу после заморозки FilmHub, меня пригласи на разработку двух Telegram приложений: Tonex farm и Casper garant.",
  ...
  },
  {
    ...
    description: [
      ...
      "Комментарии под каждым новым видео «Как составить резюме» или «Как найти работу?» заставляли меня ассоциировать поиск с чем-то невозможным...",
      "Негатив в сторону рынка был везде, в чатах, на форумах, под каждым видео в комментариях. И в целом это оправдано.",
      "Но мне всё же удалось найти работу не за хардкорные 7 месяцев, не за год и не 1,5, а относительно быстро — за 3 месяца с учётом подготовки.",
